...
// -------------------- Disziplinen / FKs / TKs --------------------
export function parseDisziplinen(selection, texts, fullText = "") {
  ...
  // --- NEU: Variante 2 erkennen
  if (fullText.includes("Disziplinen-Sortierung ändern")) {
    const blocks = {};
    const re = /([A-Za-zäöüÄÖÜß ]+)\s+-.*?Potenzial:.*?\((\d+)\sPunkte\)[\s\S]*?(?=(?:[A-Za-zäöüÄÖÜß ]+\s+-)|Impressum|$)/g;
    let m;
    while ((m = re.exec(fullText)) !== null) {
      const name = m[1].trim();
      const punkte = parseInt(m[2], 10);
      const block = m[0];
      blocks[name] = { punkte, block };
    }

    // nur die gewünschten Disziplinen zurückgeben
    const selected = dArr.filter(d => d !== '-' && blocks[d]);
    const fks = selected.map(d => getFKs(d, blocks[d].block));
    const tks = fks.map(fk => getTK(fk));

    return {
      ok: true,
      data: {
        disziplin: selected.join(" + "),
        disziplinenTK: selected.map((d, i) => `${d} TK ${tks[i]}`),
        fks,
        tks,
        gesFK: fks.reduce((acc, fk) => acc + Number(fk[0] || 0), 0)
      }
    };
  }

  // --- ALTE Logik (Variante 1)

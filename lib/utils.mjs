...
function computeInterieur(stute, hengst) {
  ...
  const w = worsts.reduce((a,b)=>a+b,0) / worsts.length; // min
  const b = bests.reduce((a,b)=>a+b,0) / bests.length;   // max
  const m = (w + b) / 2;                                  // mittel
  return `${w.toFixed(1)} / ${m.toFixed(1)} / ${b.toFixed(1)}`;
}
...
function sizeRange(stute, hengst) {
  const ms = parseNum(stute["Größe"] ?? stute.Groesse);
  const hs = parseNum(hengst["Größe"] ?? hengst.Groesse);
  if (!isNumber(ms) || !isNumber(hs)) return "—";
  const avg = (ms + hs) / 2;
  const min = Math.round(avg - 5);
  const mid = Math.round(avg);
  const max = Math.round(avg + 5);
  return `${min} / ${mid} / ${max}`;
}
...
export function computeFoalRow(stute, hengst) {
  // ✨ kein Shadowing: wir rufen die Funktion sizeRange(...) und speichern das Ergebnis in sizeStr
  const sizeStr = sizeRange(stute, hengst);
  ...
  return {
    ...
    "Größe": sizeStr, // nutzt die neue min/mittel/max-Logik aus sizeRange()
    ...
    // Interieur & Exterieur (bereits min/mittel/max im Helper umgesetzt)
